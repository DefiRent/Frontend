...
import { RENT_TICKER, RSHARE_TICKER, FTM_TICKER } from '../../../utils/constants';
...
const ZapModal: React.FC<ZapProps> = ({ onConfirm, onDismiss, tokenName = '', decimals = 18 }) => {
  ...
  const ftmAmountPerLP = tokenName.startsWith(RENT_TICKER) ? rentLPStats?.ftmAmount : rshareLPStats?.ftmAmount;
  /**
  ...
  const handleChangeAsset = (event: any) => {
    ...
    if (event.target.value === RSHARE_TICKER) {
      setZappingTokenBalance(getDisplayBalance(rshareBalance, decimals));
    }
    if (event.target.value === RENT_TICKER) {
      setZappingTokenBalance(getDisplayBalance(rentBalance, decimals));
    }
  };
  ...
  const handleChange = async (e: any) => {
    ...
    setVal(e.currentTarget.value);
    const estimateZap = await rentControl.estimateZapIn(zappingToken, tokenName, String(e.currentTarget.value));
    setEstimate({ token0: estimateZap[0].toString(), token1: estimateZap[1].toString() });
  };

  const handleSelectMax = async () => {
    setVal(zappingTokenBalance);
    const estimateZap = await rentControl.estimateZapIn(zappingToken, tokenName, String(zappingTokenBalance));
    setEstimate({ token0: estimateZap[0].toString(), token1: estimateZap[1].toString() });
  };
  ...
  return (
    <Modal>
      ...
      >
        ...
        {/* Rent as an input for zapping will be disabled due to issues occuring with the Gatekeeper system */}
        {/* <StyledMenuItem value={RENT_TICKER}>RENT</StyledMenuItem> */}
      </Select>
      <TokenInput
      ...
      <StyledDescriptionText>
        ...
        {tokenName.startsWith(RENT_TICKER) ? RENT_TICKER : RSHARE_TICKER}){' '}
      </StyledDescriptionText>
      <ModalActions>
        <Button
          color="primary"
