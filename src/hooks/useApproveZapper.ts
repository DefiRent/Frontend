...
import { FTM_TICKER, RENT_TICKER, RSHARE_TICKER, ZAPPER_ROUTER_ADDR } from '../utils/constants';
...
function useApproveZapper(zappingToken: string): [ApprovalState, () => Promise<void>] {
  ...
  let token: ERC20;
  if (zappingToken === FTM_TICKER) token = rentControl.FTM;
  else if (zappingToken === RENT_TICKER) token = rentControl.RENT;
  else if (zappingToken === RSHARE_TICKER) token = rentControl.RSHARE;
  ...
  const approvalState: ApprovalState = useMemo(() => {
    ...
    if (token === rentControl.FTM) return ApprovalState.APPROVED;
  ...
  }, [currentAllowance, pendingApproval, token, rentControl]);
